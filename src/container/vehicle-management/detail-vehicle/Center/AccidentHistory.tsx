...
const AccidentHistory = (props: any) => {
  const modelTypes = ['', '승용', '승합', '화물', '특수'];
  const useTypes = ['', '관용', '자가용', '영업용', '개인택시'];
  const {detailVehicleMgtStore} = props;
  const {manufacturer, modelDetail, insuranceHistoryList} = detailVehicleMgtStore;
  const {
    // 1. 중고차 사고이력 정보(요약)
...
export default inject('detailVehicleMgtStore')(observer(AccidentHistory));
