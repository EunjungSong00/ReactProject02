      ...
      {data ? (
        <>
          ...
          <Section border="1px solid #cccccc">
            ...
            <Line m="10px 0 16px 0" color="d8d8d8" />
            <Wrapper flex between>
              {['메인1', '메인2', '메인3', '메인4'].map((item: string, index: number) => {
                const eachImage: any = {
                  메인1: data.imageList.filter((img: any) => img.type === 'MAIN')?.[0]?.name,
                  메인2: data.imageList.filter((img: any) => img.type === 'OUTSIDE_90')?.[0]?.name,
                  메인3: data.imageList.filter((img: any) => img.type === 'OUTSIDE_180')?.[0]?.name,
                  메인4: data.imageList.filter((img: any) => img.type === 'OUTSIDE_270')?.[0]?.name
                };
            ...
            <Wrapper flex between>
              {['내부1', '내부2', '내부3', '내부4', '내부5', '내부6'].map((item: string, index: number) => {
                const eachImage: any = {
                  내부1: data.imageList.filter((img: any) => img.type === 'INSIDE_SEAT')?.[0]?.name,
                  내부2: data.imageList.filter((img: any) => img.type === 'INSIDE_CLUSTER')?.[0]?.name,
                  내부3: data.imageList.filter((img: any) => img.type === 'INSIDE_GEAR')?.[0]?.name,
                  내부4: data.imageList.filter((img: any) => img.type === 'INSIDE_HANDLE')?.[0]?.name,
                  내부5: data.imageList.filter((img: any) => img.type === 'INSIDE_NAVIGATION')?.[0]?.name,
                  내부6: data.imageList.filter((img: any) => img.type === 'INSIDE_GAS')?.[0]?.name
                };
            ...
            <Wrapper flex between>
              {['좌석1', '좌석2', '좌석3', '좌석4', '엔진룸', '타이어'].map((item: string, index: number) => {
                const eachImage: any = {
                  좌석1: data.imageList.filter((img: any) => img.type === 'SEAT_DRIVER_DOOR_OPEN')?.[0]?.name,
                  좌석2: data.imageList.filter((img: any) => img.type === 'SEAT_PASSENGER_DOOR_OPEN')?.[0]?.name,
                  좌석3: data.imageList.filter((img: any) => img.type === 'SEAT_REAR_SEAT_DOOR_OPEN')?.[0]?.name,
                  좌석4: data.imageList.filter((img: any) => img.type === 'SEAT_DRIVER_CONTROL_BUTTON')?.[0]?.name,
                  엔진룸: data.imageList.filter((img: any) => img.type === 'ENGINE_ROOM')?.[0]?.name,
                  타이어: data.imageList.filter((img: any) => img.type === 'TIRE_FRONT_RIGHT')?.[0]?.name
                };
                return (
                  <React.Fragment key={index}>
                    <Wrapper width="150px" column h>
